...
public class Senior_Design extends PApplet {
...
public void setup(){ // Setup function, similar to one in Arduino
...
public void add(){
  ...
  if(schedule_list.size() % 2 == 0){
    ...
    norm_value_add = PApplet.parseInt(convert_to_steps(PApplet.parseInt(get_input("input")), cp5_main.get(ScrollableList.class, "rotation_type").getLabel()));
  }else{
    ...
    norm_value_add = PApplet.parseInt(convert_to_ms(PApplet.parseInt(get_input("input")), cp5_main.get(ScrollableList.class, "pause_type").getLabel()));
  }
...
public void load(){
  ...
  for(int k = 0; k < loaded_schedule.length; k++){
    int convert = PApplet.parseInt(temp[k][0]);
...
public String create_schedule_str(StringList schedule){
...
public void rotate(){
  int temp = convert_to_steps(PApplet.parseInt(get_input("input")),cp5_main.get(ScrollableList.class, "rotation_type").getLabel());
...
public int convert_to_steps(int input, String type){
 float steps = 0.0f;
 if(type.equals("Radians")){
   steps = 254.65f * PApplet.parseFloat(input);
 }else if(type.equals("Degrees")){
   steps = 4.4445f * PApplet.parseFloat(input);
 }
 return PApplet.parseInt(steps);
}

public int convert_to_ms(int pause, String type){
  int delay = 0;
  //print(type + "\n");
...
public String get_input(String fieldname){
...
public void clear(){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Senior_Design" };
    if (passedArgs != null) {
